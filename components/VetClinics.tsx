...
import { findNearbyVets } from '../services/gemini';
import { VetClinic, LocationState, ServiceType, SavedProfessional } from '../types';
...
const VetClinics: React.FC<VetClinicsProps> = ({ location, onToggleFavorite, favorites }) => {
  ...
  useEffect(() => {
    const fetchVets = async () => {
      ...
      const result = await findNearbyVets(location.lat, location.lng, cityParam);
      setVets(result.vets);
      setLoading(false);
    };
    fetchVets();
  ...
  const handleCall = (vet: VetClinic) => {
    window.location.href = `tel:${vet.phone}`;
  };

  const handleWhatsApp = (vet: VetClinic) => {
    const text = encodeURIComponent(`היי, הגעתי דרך אפליקציית GoDog בנוגע לכלב שלי. האם המרפאה פתוחה?`);
    window.open(`https://wa.me/${vet.phone.replace(/\D/g, '')}?text=${text}`, '_blank');
  };

  if (loading) return <div className="p-20 text-center animate-pulse font-bold text-red-400">מחפש מרפאות...</div>;

  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {vets.map((vet, i) => {
        const isFavorited = favorites.some(f => f.name === vet.name);
        return (
          <div key={i} className="bg-white p-8 rounded-[40px] border border-gray-100 shadow-sm hover:shadow-xl transition-all flex flex-col">
            <div className="flex justify-between items-start mb-4">
               ...
               <button 
                  onClick={() => onToggleFavorite(vet.name, 'VETERINARY', vet.phone)} 
            ...
            <h4 className="text-xl font-black text-gray-800 mb-1">{vet.name}</h4>
            <p className="text-sm text-gray-400 mb-6 flex-grow">{vet.address}</p>
            <div className="flex flex-col gap-2">
              <button 
                onClick={() => handleCall(vet)}
                className="w-full bg-red-600 text-white py-4 rounded-2xl font-black flex items-center justify-center gap-2 hover:bg-red-700 transition-all shadow-lg active:scale-95"
              >
